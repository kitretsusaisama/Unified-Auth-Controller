...
async fn run_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let basic_access = sub_service
        .check_feature_access(tenant_id, "basic_access")
        .await?;
    assert!(basic_access, "Free plan should have basic_access");

    let advanced_access = sub_service
    ...
    assert!(
        can_add_user,
        "Should be able to add user (usage 0 < limit 5)"
    );
    ...
    assert!(
        can_add_user_more,
        "Should be able to add 2 more users (usage 3+2 <= 5)"
    );
    ...
    assert!(
        !can_add_over_limit,
        "Should NOT be able to add user (usage 5 >= limit 5)"
    );
    ...
    let upgraded_access = sub_service
        .check_feature_access(tenant_id, "advanced_reporting")
        .await?;
