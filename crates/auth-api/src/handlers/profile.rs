...
pub async fn complete_profile(
    ...
    let user_id = Uuid::parse_str(&claims.sub).map_err(|_| ApiError::new(AuthError::TokenError { kind: TokenErrorKind::Invalid }))?;
    
    // 1. Update Password if provided
    if let Some(password) = payload.password {
        if password.len() < 8 {
             return Err(ApiError::new(AuthError::ValidationError { message: "Password must be at least 8 chars".to_string() }));
        }
            identity_service.update_password(user_id, password).await.map_err(|_| ApiError::new(AuthError::InternalError))?;
    }

    // 2. Fetch current user to merge profile
    let mut user = identity_service.get_user(user_id).await.map_err(|_| ApiError::new(AuthError::InternalError))?;
    ...
    if let serde_json::Value::Object(mut current_map) = user.profile_data {
        ...
        // 4. Update User
        user = identity_service.update_profile(user_id, merged_profile).await.map_err(|_| ApiError::new(AuthError::InternalError))?;
    }
