...
impl IntoResponse for ApiError {
    fn into_response(self) -> Response {
        ...
        let (status, message) = match &self.inner {
            AuthError::AuthenticationFailed { reason } => (StatusCode::UNAUTHORIZED, reason.clone()),
            AuthError::AuthorizationDenied { permission, .. } => (StatusCode::FORBIDDEN, format!("Permission denied: {}", permission)),
            AuthError::TokenError { .. } => (StatusCode::UNAUTHORIZED, "Invalid or expired token".to_string()),
            AuthError::RateLimitExceeded { limit, window } => (StatusCode::TOO_MANY_REQUESTS, format!("Rate limit exceeded: {} per {}", limit, window)),
            AuthError::TenantNotFound { .. } => (StatusCode::NOT_FOUND, "Tenant not found".to_string()),
            AuthError::ConfigurationError { .. } => (StatusCode::INTERNAL_SERVER_ERROR, "Configuration error".to_string()),
            AuthError::ExternalServiceError { .. } => (StatusCode::BAD_GATEWAY, "External service error".to_string()),
            ...
            AuthError::PasswordPolicyViolation { errors } => (StatusCode::BAD_REQUEST, errors.join(", ")),
            ...
            AuthError::UTCryptoError(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Cryptography error".to_string()),
...
impl From<auth_core::services::otp_service::OtpError> for ApiError {
    fn from(error: auth_core::services::otp_service::OtpError) -> Self {
        match error {
            auth_core::services::otp_service::OtpError::Invalid => ApiError::new(auth_core::error::AuthError::InvalidOtp),
            auth_core::services::otp_service::OtpError::Expired => ApiError::new(auth_core::error::AuthError::OtpExpired),
            auth_core::services::otp_service::OtpError::NotFound => ApiError::new(auth_core::error::AuthError::InvalidOtp),
            auth_core::services::otp_service::OtpError::MaxAttemptsExceeded => ApiError::new(auth_core::error::AuthError::RateLimitExceeded { limit: 5, window: "session".to_string() }),
...
impl From<auth_core::services::otp_delivery::DeliveryError> for ApiError {
    fn from(error: auth_core::services::otp_delivery::DeliveryError) -> Self {
        ...
        match error {
            DeliveryError::CircuitBreakerOpen(s) => ApiError::new(auth_core::error::AuthError::CircuitBreakerOpen { service: s }),
            _ => ApiError::new(auth_core::error::AuthError::ExternalServiceError { service: "otp_delivery".to_string(), error: error.to_string() }),
        }
    }
}