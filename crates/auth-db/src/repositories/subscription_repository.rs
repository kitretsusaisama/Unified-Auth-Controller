...
impl SubscriptionStore for SubscriptionRepository {
    ...
    async fn get_by_tenant(&self, tenant_id: Uuid) -> Result<Option<TenantSubscription>, AuthError> {
        ...
        if let Some(row) = row {
            let id_str: String = row.try_get("id").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?;
            let t_id_str: String = row.try_get("tenant_id").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?;
            
            Ok(Some(TenantSubscription {
                id: Uuid::parse_str(&id_str).unwrap_or_default(),
                tenant_id: Uuid::parse_str(&t_id_str).unwrap_or_default(),
                plan_id: row.try_get("plan_id").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?,
                status: row.try_get("status").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?,
                start_date: row.try_get("start_date").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?,
                end_date: row.try_get("end_date").unwrap_or(None),
                current_usage: row.try_get("current_usage").map_err(|e| AuthError::DatabaseError { message: e.to_string() })?,
    ...
    async fn update_usage(&self, tenant_id: Uuid, usage: HashMap<String, i64>) -> Result<(), AuthError> {
        ...
        let sub = self.get_by_tenant(tenant_id).await?
            .ok_or(AuthError::ValidationError { message: "No subscription found".to_string() })?;

        sqlx::query("UPDATE tenant_subscriptions SET current_usage = ?, updated_at = NOW() WHERE id = ?")
