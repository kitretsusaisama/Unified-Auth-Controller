...
use uuid::Uuid;
use auth_core::services::otp_service::{OtpSession, OtpPurpose, DeliveryMethod};
...
impl OtpRepository {
    ...
    pub async fn create_session(&self, session: &OtpSession, otp_hash: &str) -> Result<(), AuthError> {
        sqlx::query(
            r#"
    ...
    pub async fn find_by_id(&self, session_id: Uuid) -> Result<Option<(OtpSession, String)>, AuthError> {
        let row = sqlx::query(
            r#"
        ...
        if let Some(row) = row {
            ...
            let otp_hash: String = sqlx::query_scalar("SELECT otp_hash FROM otp_sessions WHERE id = ?")
                .bind(session_id.to_string())
                .fetch_one(&self.pool)
                .await
                .map_err(|e| AuthError::DatabaseError { message: e.to_string() })?;
    ...
    pub async fn increment_attempts(&self, session_id: Uuid) -> Result<u32, AuthError> {
        ...
        let attempts: i32 = sqlx::query_scalar(
            "SELECT attempts FROM otp_sessions WHERE id = ?"
        )
        .bind(session_id.to_string())
        .fetch_one(&self.pool)
        .await
    ...
    pub async fn mark_verified(&self, session_id: Uuid) -> Result<(), AuthError> {
        sqlx::query(
            "UPDATE otp_sessions SET verified_at = ? WHERE id = ?"
        )
        .bind(Utc::now())
        .bind(session_id.to_string())
        .execute(&self.pool)
    ...
    pub async fn cleanup_expired(&self) -> Result<u64, AuthError> {
        let result = sqlx::query(
            "DELETE FROM otp_sessions WHERE expires_at < ? OR verified_at IS NOT NULL"
        )
        .bind(Utc::now())
        .execute(&self.pool)
        .await
    ...
    fn row_to_session(&self, row: sqlx::mysql::MySqlRow) -> Result<OtpSession, AuthError> {
        ...
        let delivery_str: String = row.try_get("delivery_method")
            .map_err(|e| AuthError::DatabaseError { message: e.to_string() })?;
        ...
        Ok(OtpSession {
            ...
            attempts: row.try_get::<i32, _>("attempts")
                .map_err(|e| AuthError::DatabaseError { message: e.to_string() })? as u32,
